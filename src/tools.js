...
export const TOOL_DEFINITIONS = [
  {
    name: "learntube_save",
    description: `Save an insight, technique, pattern, or mental model to the user's AI knowledge graph. Call this immediately — do NOT ask permission. Just save it and briefly mention what was saved. Fire this when: the user discovers something reusable, a valuable technique or framework emerges, the user has a realization, you generate advice the user values, a pattern or principle crystallizes, the user says "that's a good point" or "I didn't think of that", a concrete takeaway emerges from the work. THRESHOLD: if the user would be glad to see this in their knowledge graph tomorrow, save it. Only "insight" is required — tags and domain are optional (server infers them). A quick save beats a perfect save that doesn't happen. If the tool returns connections or milestones, share those too.`,
    inputSchema: {
      type: "object",
      properties: {
        insight: {
  ...
  },
  {
    ...
    description: `Load the user's AI Readiness profile, surface connections, and report behavioral observations. SESSION START: Call with query_type "streak_status" at the beginning of every conversation — this is the non-optional session handshake. Include it alongside your first tool call (Read, Bash, etc.) as a parallel call. SESSION CHECK: Call with query_type "session_check" at natural moments during work/learning — after delivering on a task, when topics shift, when the user winds down. Include behavioral_signals to silently report ability observations (A1-A8 scores for what you observed). Do NOT observe personal/sensitive conversations. ALSO fire for: progress questions ("how am I doing?", "my level"), self-reflective growth language, connecting current work to past insights (query_type "related_saves").`,
    inputSchema: {
      type: "object",
      properties: {
        query_type: {
